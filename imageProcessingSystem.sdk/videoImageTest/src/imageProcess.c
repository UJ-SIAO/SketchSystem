...
int initImgProcessSystem(imgProcess *imgProcessInstance, u32 axiDmaBaseAddress,XScuGic *Intc){
	...
	XScuGic_SetPriorityTriggerType(Intc,XPAR_FABRIC_SKETCH_IP_0_O_INTR_INTR,0xA0,3);
	status = XScuGic_Connect(Intc,XPAR_FABRIC_SKETCH_IP_0_O_INTR_INTR,(Xil_InterruptHandler)imageProcISR,(void *)imgProcessInstance);

	if(status != XST_SUCCESS){
		xil_printf("Interrupt connection failed");
		return -2;
	}
	XScuGic_Enable(Intc,XPAR_FABRIC_SKETCH_IP_0_O_INTR_INTR);
...
static void imageProcISR(void *CallBackRef){
	static int i=4;
	int status;
	XScuGic_Disable(((imgProcess*)CallBackRef)->IntrCtrlPointer,XPAR_FABRIC_SKETCH_IP_0_O_INTR_INTR);
	...
	XScuGic_Enable(((imgProcess*)CallBackRef)->IntrCtrlPointer,XPAR_FABRIC_SKETCH_IP_0_O_INTR_INTR);
}
